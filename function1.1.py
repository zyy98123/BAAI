#这段代码生成的negativ item全都一样，positive item正常
...
# 函数：处理每对文件组合
def process_combination(graph_file1, graph_file2, input_folder_path):
    ...
    for func_name1, graph1 in graphs1.items():
        if func_name1 in graphs2:
            graph2 = graphs2[func_name1]
            if base1 == base2 and opt_level1 != opt_level2:
                # 正例：基名相同，函数名相同，但优化等级不同
                ...
                for func_name1_neg, graph1_neg in graphs1.items():
                    for func_name2_neg, graph2_neg in graphs2.items():
                        if func_name1_neg != func_name2_neg or base1 != base2 or opt_level1 == opt_level2:
                            adj_tensor_neg1, attr_tensor_neg1 = graph_to_tensors(graph1_neg)
                            adj_tensor_neg2, attr_tensor_neg2 = graph_to_tensors(graph2_neg)
                            label = -1
                            results.append(
                                (adj_tensor_neg1, attr_tensor_neg1, adj_tensor_neg2, attr_tensor_neg2, label))
                            found_negative = True
                            break
                    if found_negative:
                        break
...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(process_combination, graph_file1, graph_file2, input_folder_path)
            for graph_file1, graph_file2 in combinations(graphs_files, 2)
        ]

        for future in tqdm(as_completed(futures), total=total_combinations, desc="Processing"):
            results = future.result()
            for result in results:
                adj_tensor1, attr_tensor1, adj_tensor2, attr_tensor2, label = result
                if label == 1:
                    positive_counter += 1
                    sample_counter += 1
                    output_file = os.path.join(output_folder_path, f'positive_sample_{positive_counter}.pkl')
                else:
                    negative_counter += 1
                    sample_counter += 1
                ...
                print(
                    f'Saved {"positive" if label == 1 else "negative"} sample {positive_counter if label == 1 else negative_counter}')

    print(
        f"共生成 {sample_counter} 个样本，其中正例 {positive_counter} 个，负例 {negative_counter} 个，每个样本已单独保存为pickle文件")
